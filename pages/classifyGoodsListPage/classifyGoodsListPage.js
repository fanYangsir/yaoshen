...
Page({
  ...
  onLoad: function (options) {
    ...
    this.setData({
      ...
      "classifyGoodsList.customFeature.isHideStock": options.isHideStock == "true" ? true : false,
      "classifyGoodsList.customFeature.isHideSales": options.isHideSales == "true" ? true : false,
      "classifyGoodsList.customFeature.isShowVirtualPrice": options.isShowVirtualPrice == "false" ? false : true,
  ...
  getClassifyGoodsListData: function () {
    ...
    let param = {
      ...
      idx_arr: idx,
      is_integral: that.data.integral,
    ...
    if (param.form == 'new_appointment'){
      app.sendRequest({
        ...
        success(res) {
          ...
          if (!param.tpl_id) {
            let noAppointTpl = {};
            noAppointTpl['goodsList'] = [];
            noAppointTpl['loadData.nomore'] = 0;
            that.setData(noAppointTpl)
            return
          }
          app.sendRequest({
  ...
  turnToAdvanceSearch: function(){
    let customFeature = this.data.classifyGoodsList.customFeature;
    let param = '&isHideStock=' + customFeature.isHideStock + '&isHideSales=' + customFeature.isHideSales + '&isShowVirtualPrice=' + customFeature.isShowVirtualPrice + '&isShoppingCart=' + customFeature.isShoppingCart + '&isBuyNow=' + customFeature.isBuyNow;
    if (this.data.categoryId){
      param += '&category=' + this.data.categoryId;
    }
    app.turnToPage('/default/pages/advanceSearch/advanceSearch?search=' + this.data.search + param);
  },
  /**
   * 综合排序 tab = 0
   */
