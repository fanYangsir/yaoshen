...
Page({
    ...
    app.sendRequest({
      ...
      success: function(res){
        ...
        for (var i = 0; i <= delivery_arr.length - 1; i++) {
          var goods_id = goods_info[i].id || goods_info[i].goods_id,
          ...
          for (var j = 0; j <= delivery_info.length - 1; j++) {
            if(delivery_info[j].is_hidden == 1){        // is_hidden = 1 时不隐藏字段
              additional_info_obj[goods_id].push({
                ...
                is_required: delivery_info[j].is_required,
                value: ''
              })
            }
          }
  ...
  submitAdditionalInfo: function(){
    ...
    for(var goodsId in additional_info_obj){
      ...
      if(additional_info_obj[goodsId]){
        ...
        for (var i = 0; i <= goods_additional_info.length - 1; i++) {
          if ( (goods_additional_info[i].is_required == 0) && (!goods_additional_info[i].value || goods_additional_info[i].value.length == 0) ){
            app.showModal({
              content: goods_additional_info[i].title + '不能为空'
            })
            completeInfo = false;
            break;
          }
    ...
    prePage.setData({
      additional_info: this.data.additional_info_obj,
      deliverydWrite: true
    })
    // typeof prePage.submitAdditionalInfoCallback == 'function' && prePage.submitAdditionalInfoCallback(this.data.additional_info_obj);
    app.turnBack();
  },
